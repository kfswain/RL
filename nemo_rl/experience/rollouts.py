...
def run_async_multi_turn_rollout(
    ...
    async def _async_rollout_implementation():
        ...
        if policy_generation.scheduler is not None:
            ...
            assigned_endpoints = {}
            for i in range(policy_generation.worker_group.dp_size):
                leader_index = policy_generation.worker_group.get_dp_leader_worker_idx(i)
                assigned_endpoints[leader_index] = Endpoint(name=leader_index, attributes={"trajectory": None, "requests_since_metric_update": 0, "num_pending_samples": 0, "kv_cache_usage_perc": 0.0, "generation_tokens": 0, "last_updated": time.time()})
